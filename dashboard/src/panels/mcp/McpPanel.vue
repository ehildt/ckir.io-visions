...
import McpPanelRequestPanel from './McpPanel.RequestPanel.vue';
import { useMcpPanel } from './use-mcp-panel';
...
const storage = useLocalStorage(
  ...
  ],
  {
    ...
    numCtx: '16384',
    task: 'describe',
    arguments_: 'Describe this image',
    stream: true,
  },
  'mcp-fields',
);
...
const requestId = computed(() => appStore.mcpRequestId);
const refreshRequestId = appStore.refreshMcpRequestId;
...
watch(analyzeButtonHover, (hovering) => {
  ...
  if (needsSubscription) startConnectButtonBlink();
  else stopConnectButtonBlink();
});
