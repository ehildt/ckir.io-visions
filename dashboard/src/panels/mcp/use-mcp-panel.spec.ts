...
describe('useMcpPanel', () => {
  ...
  it('initializes with correct defaults', () => {
    const result = useMcpPanel(createOptions());
  ...
  it('starts file field blink on hover when files missing', () => {
    const result = useMcpPanel(createOptions());
    result.setAnalyzeButtonHover(true);
    expect(mockBlink.start).toHaveBeenCalled();
  });
  ...
  it('submit succeeds with mocked fetch', async () => {
    ...
    const result = useMcpPanel(options);
    result.setFiles([new File([''], 'test.png')]);
  ...
  it('submit returns early when files missing', async () => {
    const { requireFiles } =
      await import('../../composables/use-validation.js');
    vi.mocked(requireFiles).mockReturnValueOnce(false);
  ...
  it('submit returns early for invalid task', async () => {
    const options = createOptions();
    options.task.value = 'invalid';
    const result = useMcpPanel(options);
    result.setFiles([new File([''], 'test.png')]);
    globalThis.fetch = vi.fn();
