...
import { BullMQConfigAdapter } from "./bullmq-config.adapter";
...
describe("BullMQConfigAdapter", () => {
  ...
  it("uses default values when env vars are not provided", () => {
    ...
    expect(config.defaultJobOptions.backoff?.type).toBeUndefined();
    expect(config.defaultJobOptions.backoff?.delay).toBe(5270);
  });
